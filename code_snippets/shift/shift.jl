...
function letter2angleDeg(letter::Char)::Flt
    @assert letter in 'A':'Z' "letter must be in [A-Z] set"
    return (findfirst(letter, join('A':'Z')) - 1) * 360 / 26
end

# xs, ys for letters spred evenly at the edge of the disc
function getXsAndYsForLetters(radius::Flt)::Tuple{Vec{Flt}, Vec{Flt}}
    @assert 0 <= radius <= 1 "radius must be in range [0, 1]"
    xs = Flt[]
    ys = Flt[]
    for letter in 'A':'Z'
        angDeg = letter2angleDeg(letter)
...
function drawCodingDiscs()::Cmk.Figure
    ...
    Cmk.text!(ax, getXsAndYsForLetters(0.58)..., text=shifts, color=:grey,
...
englishLettersFreqs = Dict(
    c => wikiFreqs[i] for (i, c) in enumerate('A':'Z'))

function drawFreqComparison(
    englishFreqs::Dict{Char, Flt},
    encryptMsgFreqs::Dict{Char, Flt},
    ylabEnglish::Str="plain English",
    ylabEncrypted::Str="encrypted Message",
    shift::Int=13)::Cmk.Figure
    ...
    ax1::Cmk.Axis = Cmk.Axis(fig[1, 1], xlabel="Frequency", xticks=0.0:0.05:1.0,
                             ...
                             ygridvisible=false)
    ax2::Cmk.Axis = Cmk.Axis(fig[1, 1], ylabel=ylabEncrypted,
...
drawFreqComparison(englishLettersFreqs, codedLettersFreqs)
