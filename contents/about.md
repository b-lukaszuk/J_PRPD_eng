...
likely in the range of easy to moderate) that are scattered haphazardly through
the chapters. The exercises are for the problems that, for whatever reason, I
found interesting and/or suitable. The tasks are possibly self-contained and
accompanied by exemplary solutions in [Julia](https://julialang.org/) (with
explanations). Despite that, I recommend you try to solve the assignments
yourself. Alternatively you may read the solutions and try to recreate them as
much as you can on your own. The key thing is, if you want to learn, write the
code.
